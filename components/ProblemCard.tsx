...
const ProblemCard = ({index, problem}: Props) => {
    let problemText = ""
    if (problem.current_point >= (problem.solved_criterion ?? problem.current_point)) {
    ...
    return (
        <Link href={`/problems/${problem.code}`}
            ...
            <div>
                <div className={`text-right ${problemText}`}>{problem.current_point}/{problem.point}pt</div>
