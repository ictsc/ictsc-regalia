...
デモ中は、問題コード末尾の番号に応じて「未回答・満点・部分点・15分待ち」の表示を繰り返します。番号がないコードは文字コードの合計で割り当てます。未回答・満点・部分点のサンプルは待ち時間なしです。実際の提出・採点状態を優先し、実際の待ち時間が長い場合はそちらを表示します。デモ用アイコンは表示のみで、回答・採点データや得点は変更しません。
