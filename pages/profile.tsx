...
const Profile = () => {
  ...
  const getCurrentValue = () => ({
    display_name: (user?.display_name ?? "") == "" ? user?.name : user?.display_name,
      ...
      <>
        ...
        <div className={'container-ictsc pt-8'}>
          <ICTSCCard>
            ...
            {status === 202 &&
              <ICTSCSuccessAlert message={'プロフィールを更新しました'} className={'mb-8'}/>}
            {status != null && status !== 202 &&
              <ICTSCErrorAlert message={'プロフィールの更新に失敗しました'} className={'mb-8'}/>}
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className={'form-control'}>
                ...
                <input {...register(
                    ...
                    {required: true})
                       ...
                       type="text" placeholder="名前" className={'input input-bordered'}/>
                {errors.display_name && <p className={'label-text-alt text-error mt-1'}>表示名は必須です</p>}
              </div>
              ...
              <div className={'form-control pt-6'}>
                ...
                <Controller name={'self_introduction'} control={control} render={({field}) => (
                    <textarea {...field} className={'textarea h-24 textarea-bordered'} placeholder="自己紹介"/>
                )}/>
              </div>
              <div className={'form-control pt-4'}>
                <label className={'label'}>
                ...
                <input  {...register("github_id")}
                        type="text" placeholder="ユーザー名のみを入力" className={'input input-bordered'}/>
              </div>
              <div className={'form-control pt-4'}>
                <label className={'label'}>
                  <span className={'label-text'}>Twitter ID</span>
                </label>
                <input {...register("twitter_id")}
                       type="text" placeholder="@マークなしで入力" className={'input input-bordered'}/>
              </div>
              <div className={'form-control pt-4'}>
                <label className={'label'}>
                  <span className={'label-text'}>Facebook ID</span>
                </label>
                <input  {...register("facebook_id")}
                        type="text" placeholder="ユーザー名のみを入力" className={'input input-bordered'}/>
              </div>
