        ...
        __html: markdownToHtml(content ?? "", {
          customEmbed: {
            ...
            <div class="flex items-center mb-4">
                <input id="radio-${count}-${index}" type="radio" value=${index} name="radio${count}" class="w-6 h-4 text-blue-600 bg-gray-100 border-gray-300 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600">
                <label for="radio-${count}-${index}" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">${label}</label>
            </div>`;
              });
        ...
        <div class="radio-buttons pt-2">
            ...
            ictscc(str) {
              ...
              const checkboxes = lines.map((line, index) => {
                ...
                return `
                      <div class="flex items-center mb-4">
                          <input id="checkbox-${count}-${index}" type="checkbox" value=${index} name="checkbox${count}" class="w-6 h-4 text-blue-600 bg-gray-100 border-gray-300 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600">
                          <label for="checkbox-${count}-${index}" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">${label}</label>
                      </div>`;
              });
